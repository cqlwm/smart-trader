...
class GridStrategy(Strategy):
    ...
    def __init__(self, symbol: str, master_side: OrderSide, strategy_key: str, per_qty: float = 0.02,
                 take_profit_rate=0.006, upper_limit: float = 1000000, lower_limit: float = 0):
        super().__init__()
        self.symbol = symbol
...
class SwapGridStrategy(GridStrategy):
    def __init__(self, ex_client: ExSwapClient, symbol: str, position_side: str, master_side: OrderSide,
        ...
        super().__init__(symbol, master_side, strategy_key, per_qty, take_profit_rate, upper_limit, lower_limit)
...
class SpotGridStrategy(GridStrategy):
    def __init__(self, ex_client: ExSpotClient, symbol: str, master_side: OrderSide, strategy_key: str,
                 per_qty: float = 0.02, take_profit_rate=0.006, upper_limit: float = 1000000, lower_limit: float = 0):
        super().__init__(symbol, master_side, strategy_key, per_qty, take_profit_rate, upper_limit, lower_limit)
        self.ex_client = ex_client

    def place_order(self, order_id: str, side: OrderSide, qty: float, price: float = None):
        self.ex_client.place_order(order_id, self.symbol, side.value, qty, price)
