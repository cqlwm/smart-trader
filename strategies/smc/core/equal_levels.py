...
from strategies.smc.models import EqualLevel, Pivot, Pivots


def detect_equal_levels(_pivots: Pivots, atr_series: pd.Series, df: pd.DataFrame, threshold: float = 0.1) -> list[EqualLevel]:
...
def _detect_for_pivots(pivots: list[Pivot], atr_series: pd.Series, df: pd.DataFrame, threshold: float, level_type: str) -> list[EqualLevel]:
